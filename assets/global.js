...
// Collection Tab Starts
document.addEventListener('DOMContentLoaded', function () {
  if (!document.querySelector('#collection-tabs')) return;
  ...
  tabs.forEach((tab) => {
    tab.addEventListener('click', async function () {
      ...
      const cleanUrl = `/collections/${collectionHandle}`;
      history.pushState(null, '', cleanUrl);

      // Reset internal facet state
      FacetFiltersForm.filterData = [];
      FacetFiltersForm.searchParamsInitial = '';
      FacetFiltersForm.searchParamsPrev = '';

      // Manually call the facet filter system to re-render
      FacetFiltersForm.renderPage('', null);

      // Update collection banner
      fetch(`${cleanUrl}?section_id=main-collection-banner`)
        .then((res) => res.text())
        .then((html) => {
          const doc = new DOMParser().parseFromString(html, 'text/html');
          const newBanner = doc.querySelector('.collection-hero');
          const currentBanner = document.querySelector('.collection-hero');
